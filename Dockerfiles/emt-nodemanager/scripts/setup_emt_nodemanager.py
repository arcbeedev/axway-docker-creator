...
def _checkLicense():
    mcAfeeFilterUsed = _checkFeaturesUsed()
    licenseFile = os.path.join(DISTDIR, "conf", "licenses", "lic.lic")
    if not os.path.exists(licenseFile):
        if mcAfeeFilterUsed:
            _fail("McAfee filter is configured in the FED, but no license file supplied.")
        if options.fips:
...
def _checkFeaturesUsed():
    print("Checking which system features are used:")
