...
FACTORY_FED_FILE = os.path.join(DISTDIR, "system", "conf", "templates", "FactoryConfiguration-%s.fed"
                                % vutil.getBrandingMap()["server.instanceServiceType"])
...
def _installCustomFedFile():
    if not os.path.exists(options.fedFile):
        print("Using factory FED for API Gateway")
        return
    ...
    # Overwrite factory FED template with provided FED
...
def _checkLicense():
    mcAfeeFilterUsed, apiManagerUsed = _checkFeaturesUsed()
    ...
    if mcAfeeFilterUsed and not re.search("mcafee *= *1", s):
        _fail("Supplied license file is not valid for McAfee, but a McAfee filter is configured in the FED.")
...
def _checkFeaturesUsed():
    ...
    fedDir = os.path.join(DISTDIR, "groups", GROUP_NAME, "conf", esid)

    # Check PrimaryStore.xml to see if McAfee filter and API Manager are used
    pattern = re.compile("PrimaryStore.*xml")
    primaryStore = [fname for fname in os.listdir(fedDir) if re.match(pattern, fname)][0]
    with open(os.path.join(fedDir, primaryStore)) as f:
        s = f.read()

    pattern = re.compile('<entity .* type="McAfeeVirusCheck"')
    if re.search(pattern, s):
        mcAfeeFilterUsed = True
    print("   McAfee filter: %s" % mcAfeeFilterUsed)
    ...
    return mcAfeeFilterUsed, apiManagerUsed
